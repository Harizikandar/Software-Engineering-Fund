...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'update_customer') {
            ...
            // Cascades through orders, order_details, payments, qr_codes, reviews, and sessions.
            $pdo->prepare('DELETE FROM customers WHERE cust_id=?')->execute([(int)$_POST['cust_id']]);
            $flash = 'Customer account and all of their orders, payments, and reviews have been permanently deleted.';
            ...
            // Cascades through items, orders, order_details, order_queue, payments, qr_codes, and reviews.
            $pdo->prepare('DELETE FROM vendors WHERE vendor_id=?')->execute([(int)$_POST['vendor_id']]);
            $flash = 'Vendor account and all of their menu items, orders, payments, and reviews have been permanently deleted.';
        }
    } catch (PDOException $e) {
        $error = 'Action could not be completed due to a database error.';
    }
}
...
<?php if($tab==='customers'): ?><tr><td>#<?= e($r['cust_id']) ?></td><td><?= e($r['name']) ?></td><td><?= e($r['email']) ?></td><td class="text-end"><button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#customer<?= $r['cust_id'] ?>">Edit</button><form class="d-inline" method="post" onsubmit="return confirm('Delete this customer? This permanently erases their account along with all of their orders, payments, and reviews. This cannot be undone.');"><input type="hidden" name="action" value="delete_customer"><input type="hidden" name="cust_id" value="<?= $r['cust_id'] ?>"><button class="btn btn-sm btn-outline-danger">Delete</button></form></td></tr>
