...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update_item') {
    ...
    // ---- Look up the current row so we know what to keep if no new image is uploaded ----
    $existing_stmt = $conn->prepare("SELECT image_path FROM items WHERE item_id = ? AND vendor_id = ?");
    $existing_stmt->bind_param("ii", $item_id, $vendor_id);
    ...
    // ---- Image upload is optional on edit; only replace if a new file is provided ----
    if (isset($_FILES['item_image']) && $_FILES['item_image']['error'] === UPLOAD_ERR_OK) {
        $allowed_types = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'];
        $file_type = mime_content_type($_FILES['item_image']['tmp_name']);
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare(
            "UPDATE items SET item_name = ?, item_desc = ?, category = ?, price = ?, image_path = ?, is_active = ?
             WHERE item_id = ? AND vendor_id = ?"
        );
        $stmt->bind_param("sssdsiii", $item_name, $item_desc, $category, $price, $image_path, $is_active, $item_id, $vendor_id);
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .edit-item-form-wrap { display: none !important; margin-top: -6px; margin-bottom: 14px; }
  .edit-item-form-wrap.open { display: block !important; }
    ...
    <?php foreach ($items_by_category as $category => $items): ?>
      <div class="category-box">
        ...
        <?php foreach ($items as $item): ?>
          <div class="item-box">
            <div class="item-info">
              ...
              <div class="item-controls edit-only">
                <button type="button" class="btn btn-outline btn-small" onclick="toggleItemEdit(<?php echo (int) $item['item_id']; ?>)">
                  Edit
                </button>
          ...
          <div class="edit-only edit-item-form-wrap" id="editForm-<?php echo $item['item_id']; ?>">
            <div class="add-item-form">
              ...
              <form method="POST" action="menu.php" enctype="multipart/form-data">
                ...
                <div class="form-row">
                  <label for="item_name-<?php echo $item['item_id']; ?>">Item Name (max 50 characters)</label>
                  <input type="text" id="item_name-<?php echo $item['item_id']; ?>" name="item_name" maxlength="50" required
                         value="<?php echo htmlspecialchars($item['item_name']); ?>">
                </div>

                <div class="form-row">
                  <label for="category-<?php echo $item['item_id']; ?>">Category</label>
                  <select id="category-<?php echo $item['item_id']; ?>" name="category">
                ...
                <div class="form-row">
                  <label for="item_desc-<?php echo $item['item_id']; ?>">Item Description</label>
                  <textarea id="item_desc-<?php echo $item['item_id']; ?>" name="item_desc"><?php echo htmlspecialchars($item['item_desc'] ?? ''); ?></textarea>
                </div>

                <div class="form-row">
                  <label for="price-<?php echo $item['item_id']; ?>">Price (RM)</label>
                  <input type="number" id="price-<?php echo $item['item_id']; ?>" name="price" step="0.01" min="0" required
                         value="<?php echo htmlspecialchars((string) $item['price']); ?>">
                </div>

                <div class="form-row">
                  <label for="item_image-<?php echo $item['item_id']; ?>">Replace Image (optional)</label>
                  <input type="file" id="item_image-<?php echo $item['item_id']; ?>" name="item_image" accept="image/jpeg,image/png,image/webp,image/gif">
                </div>

                <div class="form-row checkbox-row">
                  <input type="checkbox" id="is_active-<?php echo $item['item_id']; ?>" name="is_active" <?php echo $item['is_active'] ? 'checked' : ''; ?>>
                  <label for="is_active-<?php echo $item['item_id']; ?>">Available for order</label>
                </div>

                <div class="reply-form-actions" style="display:flex; gap:10px; margin-top: 8px;">
                  <button type="submit" class="btn">Save Changes</button>
                  <button type="button" class="btn btn-outline" onclick="toggleItemEdit(<?php echo $item['item_id']; ?>)">Cancel</button>
                </div>
              </form>
            </div>
          </div>
...
<script>
  ...
  function toggleItemEdit(itemId) {
    document.getElementById('editForm-' + itemId).classList.toggle('open');
  }
</script>
